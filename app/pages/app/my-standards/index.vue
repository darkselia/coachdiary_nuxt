...
definePageMeta({ name: 'my-standards', mobileTitle: 'Мои нормативы' });
...
const levelButtonText = computed(() => selectedLevelNumber.value != -1 ? selectedLevelNumber.value + ' год обучения' : 'Года обучения');
...
const levels = computed(() => Array.from(standards.value
...
const simplifiedStandards = computed(() => standards.value
  .filter(standard => standard.has_numeric_value === (pageType.value === 'standards'))
...
const currentStandardLevels = computed(() => standards.value
    ...
    >
      <template #default="{ toggle }">
        <SideNavButtons
          ...
          @type-changed="setFirst({ level: false })"
        />
        <DataTableSideNav
